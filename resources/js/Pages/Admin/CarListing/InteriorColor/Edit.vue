...
const form = useForm({
    ...
    status: props.color?.status ?? true,
})
...
const submit = () => {
    form.post(route('admin.car-listings.interior-color.update', props.color.id), {
        onSuccess: () => {
            form.reset()
            emit('close')
        }
...
<template>
    <Modal :show="show" @close="emit('close')" maxWidth="xl">
        <div class="p-8">
            <h2 class="text-xl font-bold text-gray-900 border-b pb-4 mb-8 uppercase tracking-widest text-center">Update Interior Color</h2>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t">
                    ...
                    <PrimaryButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Update Color</PrimaryButton>
                </div>
            </form>
        </div>
    </Modal>
