...
const deleteColor = (color) => {
    if (confirm(`Are you sure you want to delete ${color.name}?`)) {
        router.delete(route('admin.car-listings.interior-color.destroy', color.id))
    }
}

const toggleStatus = (color) => {
    router.post(route('admin.car-listings.interior-color.toggle-status', color.id))
}
</script>

<template>
    <AdminLayout>
        <Head title="Interior Colors" />

        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Interior Colors</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage available vehicle cabin material colors.</p>
                </div>
