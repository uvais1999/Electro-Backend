<script setup>
import { useForm } from '@inertiajs/vue3'
import Modal from '@/Components/Modal.vue'
import InputLabel from '@/Components/InputLabel.vue'
import TextInput from '@/Components/TextInput.vue'
import InputError from '@/Components/InputError.vue'
import PrimaryButton from '@/Components/PrimaryButton.vue'
import SecondaryButton from '@/Components/SecondaryButton.vue'
import { ref, watch, onMounted } from 'vue'
import axios from 'axios'

const props = defineProps({
    show: Boolean,
    trim: Object,
    makes: Array
})

const emit = defineEmits(['close'])

const selectedMakeId = ref(props.trim?.model?.car_make_id || '')
const models = ref([])
const loadingModels = ref(false)

const form = useForm({
    _method: 'PUT',
    car_model_id: props.trim?.car_model_id || '',
    name: props.trim?.name || '',
    status: props.trim?.status ?? true,
})

const fetchModels = async (makeId) => {
    if (!makeId) return
    loadingModels.value = true
    try {
        const response = await axios.get(route('admin.car-listings.trim.get-models', makeId))
        models.value = response.data
    } catch (error) {
        console.error('Error fetching models:', error)
    } finally {
        loadingModels.value = false
    }
}

watch(selectedMakeId, (newVal, oldVal) => {
    if (newVal !== oldVal) {
        if (oldVal !== '') {
            form.car_model_id = ''
        }
        fetchModels(newVal)
    }
})

watch(() => props.trim, (newVal) => {
    if (newVal) {
        selectedMakeId.value = newVal.model.car_make_id
        form.car_model_id = newVal.car_model_id
        form.name = newVal.name
        form.status = newVal.status
        fetchModels(newVal.model.car_make_id)
    }
})

onMounted(() => {
    if (selectedMakeId.value) {
        fetchModels(selectedMakeId.value)
    }
})

const submit = () => {
    form.post(route('admin.car-listings.trim.update', props.trim.id), {
        onSuccess: () => {
            form.reset()
            emit('close')
        }
    })
}
</script>

<template>
    <Modal :show="show" @close="emit('close')" maxWidth="xl">
        <div class="p-8">
            <h2 class="text-xl font-bold text-gray-900 border-b pb-4 mb-8 uppercase tracking-widest text-center">Update Car Trim</h2>
            
            <form @submit.prevent="submit" class="space-y-6">
                <div>
                    <InputLabel for="make" value="Select Make" class="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2" />
                    <select v-model="selectedMakeId" id="make" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        <option v-for="make in makes" :key="make.id" :value="make.id">{{ make.name }}</option>
                    </select>
                </div>

                <div>
                    <InputLabel for="car_model_id" value="Select Model" class="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2" />
                    <select v-model="form.car_model_id" id="car_model_id" :disabled="loadingModels" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                        <option v-for="model in models" :key="model.id" :value="model.id">{{ model.name }}</option>
                    </select>
                    <InputError :message="form.errors.car_model_id" class="mt-2" />
                </div>

                <div>
                    <InputLabel for="name" value="Trim Name" class="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2" />
                    <TextInput id="name" type="text" v-model="form.name" class="w-full" required />
                    <InputError :message="form.errors.name" class="mt-2" />
                </div>

                <div class="block">
                    <label class="flex items-center">
                        <input type="checkbox" v-model="form.status" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
                        <span class="ml-2 text-sm text-gray-600 font-bold uppercase tracking-widest text-[10px]">Active Status</span>
                    </label>
                </div>

                <div class="flex items-center justify-end space-x-3 pt-6 border-t">
                    <SecondaryButton @click="emit('close')" :disabled="form.processing">Cancel</SecondaryButton>
                    <PrimaryButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Update Trim</PrimaryButton>
                </div>
            </form>
        </div>
    </Modal>
</template>
