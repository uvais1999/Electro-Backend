...
const deleteFeature = (feature) => {
    if (confirm(`Are you sure you want to delete ${feature.name}?`)) {
        router.delete(route('admin.car-listings.tech-feature.destroy', feature.id))
    }
}

const toggleStatus = (feature) => {
    router.post(route('admin.car-listings.tech-feature.toggle-status', feature.id))
}
</script>

<template>
    <AdminLayout>
        <Head title="Technology Features" />

        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Technology Features</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage entertainment and connectivity options (e.g. Bluetooth, USB Ports).</p>
                </div>
                <button @click="openCreateModal"
                    class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition font-medium">
                    Add Technology Feature
                </button>
            </header>
