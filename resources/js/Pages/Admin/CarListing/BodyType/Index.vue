...
const deleteType = (type) => {
    if (confirm(`Are you sure you want to delete ${type.name}?`)) {
        router.delete(route('admin.car-listings.body-type.destroy', type.id))
    }
}

const toggleStatus = (type) => {
    router.post(route('admin.car-listings.body-type.toggle-status', type.id))
}
</script>
    ...
    <AdminLayout>
        ...
        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Body Types</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage vehicle body styles (e.g. SUV, Sedan, Coupe).</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-4">Body Style Name</th>
