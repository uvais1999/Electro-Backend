...
const deleteFeature = (feature) => {
    if (confirm(`Are you sure you want to delete ${feature.name}?`)) {
        router.delete(route('admin.car-listings.comfort-feature.destroy', feature.id))
    }
}

const toggleStatus = (feature) => {
    router.post(route('admin.car-listings.comfort-feature.toggle-status', feature.id))
}
</script>

<template>
    <AdminLayout>
        <Head title="Comfort Features" />

        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Comfort Features</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage driver comfort and convenience options (e.g. Air Conditioning, Heated Seats).</p>
                </div>
