...
const deleteCapacity = (capacity) => {
    if (confirm(`Are you sure you want to delete ${capacity.name}?`)) {
        router.delete(route('admin.car-listings.engine-capacity.destroy', capacity.id))
    }
}

const toggleStatus = (capacity) => {
    router.post(route('admin.car-listings.engine-capacity.toggle-status', capacity.id))
}
</script>

<template>
    <AdminLayout>
        <Head title="Engine Capacities" />

        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Engine Capacities</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage engine displacement (e.g. 1.6L, 2.0L, 5.0L).</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-4">Engine Capacity Name</th>
