...
const deleteType = (type) => {
    if (confirm(`Are you sure you want to delete ${type.name}?`)) {
        router.delete(route('admin.car-listings.door-type.destroy', type.id))
    }
}

const toggleStatus = (type) => {
    router.post(route('admin.car-listings.door-type.toggle-status', type.id))
}
</script>

<template>
    <AdminLayout>
        <Head title="Door Types" />

        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Door Types</h1>
                    <p class="text-gray-500 text-sm mt-1">Manage door configurations (e.g. 2 Doors, 4 Doors).</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-4">Door Configuration Name</th>
