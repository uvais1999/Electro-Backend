    ...
    <AdminLayout>
        ...
        <div class="space-y-6">
            <header class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <Link :href="route('admin.used-car-listings.create')"
                        ...
                        <tr v-for="(listing, index) in listings.data" :key="listing.id"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-4">
                                    <div
                                        class="w-20 h-14 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden shadow-sm flex-shrink-0">
                                        <img v-if="getMainImage(listing)" :src="getMainImage(listing)"
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div
                                            class="flex items-center space-x-2 text-[10px] text-gray-500 font-bold uppercase tracking-wider mt-0.5">
                                            ...
                                            <span class="text-gray-300">•</span>
                                            <span
                                                :class="[listing.condition_type === 'new' ? 'text-rose-600' : 'text-emerald-600']">{{
                                                    listing.condition_type }}</span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center text-sm font-medium text-gray-700">
                                ...
                                <div class="text-[9px] uppercase font-black text-gray-400 tracking-widest mt-0.5">{{
                                    listing.seller_type || 'Buyer' }}</div>
                            </td>
                            ...
                            <!-- Verified -->
                            <td class="px-6 py-4 text-center">
                                <span v-if="listing.status === 'approved'" class="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border border-blue-200">
                                    Verified
                                </span>
                                <span v-else-if="listing.status === 'rejected'" class="bg-rose-50 text-rose-600 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border border-rose-200">
                                    Rejected
                                </span>
                                <span v-else class="bg-amber-50 text-amber-600 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border border-amber-200">
                                    Pending
                                </span>
                            </td>
                            ...
                            <!-- Actions -->
                            <td class="px-6 py-4 text-center">
                                <Link :href="route('admin.used-car-listings.show', listing.id)"
            ...
            <!-- Empty State -->
            <div v-else
                class="bg-white p-16 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center space-y-4">
                ...
                <Link :href="route('admin.used-car-listings.create')"
                    class="bg-indigo-600 text-white px-8 py-2.5 rounded-lg shadow-lg shadow-indigo-100 font-bold transition hover:bg-indigo-700">
                    Post My First Used Ad
                </Link>
            </div>
        </div>
    </AdminLayout>
