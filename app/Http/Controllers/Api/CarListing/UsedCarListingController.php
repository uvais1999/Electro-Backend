...
{
    /**
     * Display a listing of used car listings with filters.
     */
    public function index(Request $request)
    {
        $query = CarListing::with(['images', 'make', 'model', 'emirate', 'user', 'user.roles'])
    ...
    /**
     * Get master data for used car search filters.
     */
    ...
    /**
     * Get data required for creating a used car listing form.
     */
    ...
    /**
     * Display the specified used car listing details.
     */
    public function show(CarListing $used_car_listing)
    {
        // Ensure it's used car
        if ($used_car_listing->condition_type !== 'used') {
            return response()->json(['success' => false, 'message' => 'Listing not found.'], 404);
        }

        // Only show if approved
        if ($used_car_listing->status !== 'approved' && !request()->user()?->hasRole('admin')) {
            return response()->json(['success' => false, 'message' => 'Listing not found or not approved.'], 404);
        }

        return response()->json([
            'success' => true,
            'data' => $used_car_listing->load([
                'images',
                'make',
                'model',
                'trim',
